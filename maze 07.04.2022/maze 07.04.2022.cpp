...
void ShowCoins(int coins, int WIDTH, COORD infobox);
void HEALTH(int health, int WIDTH, COORD infobox);
void ShowSteps(int steps, COORD infobox);
void ShowEnergy(int energy, COORD infobox);
int ALL_COINS(int allcoins, int maze[][40], int HEIGHT, int WIDTH);
...
int main()
{
	...
	HEALTH(health, WIDTH, infobox);
	ShowCoins(coins, WIDTH, infobox);
	ShowSteps(steps, infobox);
	ShowEnergy(energy, infobox);

	allcoins = ALL_COINS(allcoins, maze, HEIGHT, WIDTH);
